...
def test_sign_up_and_sign_in(client):
    ...
    sign_in_response = client.post("/v1/auth/sign-in", json=sign_in_info)
    ...
    get_me_response = client.get("/v1/auth/me", headers={"Authorization": f"Bearer {sign_in_response.json()['access_token']}"})
